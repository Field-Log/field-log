---
name: logger
description: Use when making or reviewing code changes in this repo that add, edit, or touch logging in apps/web, packages/services, or packages/database; when replacing console.*; when adding logger messages or protocol values; or before validation after code changes to audit shared logger usage.
---
...
3. Replace `console.*` in `apps/web` and `packages/services` with the shared logger.
...
- Server-side service code: use the configured service logger from the app-local services module, usually `s.logger`.
