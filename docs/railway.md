...
to the `scraper-queue` service and verifies that Railway resolves
`DATABASE_URL` plus either `REDIS_URL` or fallback `REDIS` to valid runtime URLs
before deployment. After the variables are set, the workflow deploys the
`scraper-queue` Redis service from its configured image source so the Redis
database is online for the scraper, then deploys the scraper service from its
